  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      spawner.spawn({
        ...
        message: buildUserMessage(transcript, existingPreview, hasWorkspace),
...
function buildUserMessage(transcript: string, existingPreview: readonly string[], hasWorkspace: boolean): string {
...
const EXTRACTION_SYSTEM_PROMPT = `You distill DURABLE, REUSABLE memory from a completed AI-agent run. Extract ONLY facts that will still be true and useful in FUTURE, UNRELATED runs.
