...
Unlike a [Workflow](/docs/guides/workflows) — a fixed graph of steps — a Discussion is steered live by the Facilitator. It decides how the conversation unfolds and when it is done.
...
If you instead need a deterministic, repeatable pipeline, reach for a [Workflow](/docs/guides/workflows) or [Orchestration](/docs/guides/orchestration).
...
Discussion turns run headlessly with no human in the loop, so they start the Claude Code CLI in `bypassPermissions` mode — there is nobody to answer a tool-permission prompt. Spend stays bounded by each Assistant's budget cap and [Safety Rules](/docs/guides/safety-rules). Review your [Safety Rules](/docs/guides/safety-rules) before letting tool-using Assistants debate.
</Callout>
...
See the full [API reference](/docs/reference/api) for the shared response envelope and error codes.
