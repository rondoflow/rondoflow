...
Schedules let RondoFlow run work for you on a recurring basis — no clicking required. You point a schedule at either a saved [Workflow](/docs/guides/workflows) or a single Assistant (internally an *agent*), give it a message and a cron expression, and the scheduler starts it at the times you define.
...
| `workflow` | A saved Workflow run as a **simplified sequential chain**. Each agent in the saved workflow runs in turn with its real persona and model, and each one's output is passed forward as the input to the next. There are no DAG edges, no [Condition](/docs/guides/data-nodes) branches, and no [Director](/docs/guides/orchestration) — just a straight line through the agents. |
...
When a scheduled **workflow** finishes its agents, the scheduler also honors any saved [Output and Email cards](/docs/guides/data-nodes) (Cards = Nodes) baked into the workflow. This happens headlessly — there is no canvas — so the behavior differs slightly from an interactive run:
