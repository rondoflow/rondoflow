...
The `test_pass` command is split on whitespace and run directly (never through a shell), so shell features like pipes, `&&`, globbing, or environment-variable expansion will not work. Use a single executable plus its arguments, for example `npm test`. See [Security](/docs/reference/security) for why RondoFlow never spawns a shell.
</Callout>
...
While a loop runs, the engine emits events. The in-app client subscribes to them over Socket.IO and you can also watch the broader run in [Monitoring](/docs/guides/monitoring):
...
<Callout type="info">
Loop iterations run headless with permissions bypassed so tools execute without an in-run prompt. The only human gate is the optional per-iteration approval of the `manual` criterion. Scope each Assistant's allowed tools and budget carefully — see [Safety Rules](/docs/guides/safety-rules).
</Callout>
