...
<Callout type="info">
This is a **detect-and-hold** model, not a kernel sandbox. Gating runs in the `onToolUse` callback *after* the CLI has already emitted the `tool_use` event — RondoFlow stops the Assistant and closes the session at that point. Native pre-execution blocking is on the roadmap (see [Security](/docs/reference/security)) but is not implemented today, so a blocked pattern prevents the Assistant from *continuing*, not from a tool having been requested.
</Callout>
...
The headless run paths spawn the CLI with `permissionMode: 'bypassPermissions'` and never call the policy checker, so a `blockedCommands` or `requireApproval` pattern has **no effect** on a [Workflow](/docs/guides/workflows) run, a [Loop](/docs/guides/loops), a [Discussion](/docs/guides/discussions), or a [scheduled task](/docs/guides/schedules). Their enforced guardrails are the budget cap and the spawn idle / wall-clock timeouts.
...
<Callout type="info">
Setting the budget is gated by the generic **write** capability (editor+), not by admin — so any editor can change the workspace spend cap. Compare with credential editing, which is admin-only. See [Users & roles](/docs/guides/users).
</Callout>
