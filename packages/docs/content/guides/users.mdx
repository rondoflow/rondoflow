...
Add `RONDOFLOW_ADMIN_EMAIL` and `RONDOFLOW_ADMIN_PASSWORD` (and optionally `RONDOFLOW_ADMIN_NAME`) to your environment or `.env`. See [Configuration](/docs/getting-started/configuration).

### Run the seed

The bootstrap runs **only at seed time** — `npm run db:seed`, or `npm run setup` (which seeds as part of installation). It is **not** a server-startup hook. See [Installation](/docs/getting-started/installation).
...
Each admin action writes an entry to the audit log — `user_invited`, `user_role_changed`, `user_deactivated`, `user_reactivated`, and `user_deleted` — so user-management changes are reviewable later. See [Monitoring & Audit](/docs/guides/monitoring).
...
<Cards>
  ...
  <Cards.Card title="Monitoring & Audit" href="/docs/guides/monitoring" />
