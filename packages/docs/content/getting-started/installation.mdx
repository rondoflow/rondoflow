...
| **Claude Code CLI** | Assistants (agents) are Claude Code subprocesses | Install it and authenticate so your assistants can run. See [Claude Code provider](/docs/providers/claude-code). |
...
<Callout type="info">
RondoFlow can also use OpenAI and Perplexity for assistants. Those need API keys but no CLI. See [Providers](/docs/providers).
</Callout>
...
<Callout type="info">
Setup generates a working `.env`, but the Claude credential is left blank. To run assistants on Claude, add an `ANTHROPIC_API_KEY` **or** a `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`). If both are set, the setup token wins. See [Configuration](/docs/getting-started/configuration).
</Callout>
...
To add more people, create their accounts from the [Users panel](/docs/guides/users) — only an admin can do this. New users default to the read-only `viewer` role.
...
<Cards>
  ...
  <Cards.Card title="Build your first workflow" href="/docs/getting-started/first-workflow" />
  <Cards.Card title="Claude Code provider" href="/docs/providers/claude-code" />
</Cards>
