...
RondoFlow stores all state in PostgreSQL via [Prisma](/docs/reference/architecture). This page documents the schema entity by entity: what each model is for, its key fields, and how it relates to the rest. Field names below match the database exactly — the UI wraps many of them in friendlier terms (see [Terminology](/docs/concepts/terminology)).
...
RondoFlow uses [Better Auth](/docs/reference/security). Three tables back it: `User`, `Session`, and `Account`, plus a transient `Verification` table.
...
<Callout type="info">
`role` gates a single shared team workspace — access is by role, not per-user ownership (`userId` columns are kept for attribution/audit only). The role migration backfills existing users to `editor` and promotes the oldest account to `admin`. See [Security](/docs/reference/security).
</Callout>
...
See [Assistants](/docs/guides/assistants), [Loops](/docs/guides/loops), and [Providers](/docs/providers).
...
Composite primary key `[agentId, skillId]`. See [Skills](/docs/guides/skills).
...
When multiple rules apply, the most restrictive wins. See [Safety Rules](/docs/guides/safety-rules) and [Security](/docs/reference/security).

<Callout type="info">
The global per-run budget cap is stored on a single canonical global `Policy` row (id `d0000000-0000-0000-0000-000000000001`), not in an env var. See [Security](/docs/reference/security).
</Callout>
...
Composite primary key `[tableId, agentId]`. See [Discussions](/docs/guides/discussions).
...
Unique on `[agentId, key]` and `[workspaceId, key]` (upserts rely on these). Indexed on `[workspaceId, pinned]`, `[agentId, pinned]`, and `[scope, source]`. See [Memory](/docs/guides/memory).

## Resources

### WorkspaceResource

A Resource attached to a Workspace — a file, URL, note, or variable. See [Resources](/docs/guides/resources).
...
Join table assigning a Connection to an Assistant. Composite primary key `[agentId, mcpServerId]`; both FKs cascade on delete. See [Connections](/docs/guides/connections).

## External Folders

### ExternalFolder

A host directory bind-mounted into the server and exposed to Assistants. See [External Folders](/docs/guides/external-folders).
...
A ChainRun records a workflow execution (a DAG of steps). It owns an append-only event log and per-step results. See [Workflows](/docs/guides/workflows) and [Orchestration](/docs/guides/orchestration).
...
<Callout type="info">
A chain step isn't always an Assistant turn. The Structurer (Structure) and Save to DB cards run as real non-agent steps inside the [ChainExecutor](/docs/guides/workflows); the Output and Email cards are run-completion sinks rather than execution steps. See [Data Nodes](/docs/guides/data-nodes) for the canvas-side picture and [Structured datasets](#structured-datasets) below for the tables they write.
</Callout>
...
Indexed on `[userId, createdAt]` and `[workspaceId, createdAt]`. See [Monitoring](/docs/guides/monitoring).
...
A cron-scheduled run of a saved workflow or an assistant. See [Schedules](/docs/guides/schedules).
...
<Callout type="warning">
Secrets in `Setting` and any `WorkspaceResource` with `isSecret = true` are sensitive. Review [Security](/docs/reference/security) before exposing the database or backups.
</Callout>
...
<Cards>
  ...
  <Cards.Card title="API" href="/docs/reference/api" />
