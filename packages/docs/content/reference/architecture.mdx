...
Agents run as real subprocesses or live API calls. RondoFlow never uses `shell: true` — every command is an argument array — and server secrets such as `DATABASE_URL` and `BETTER_AUTH_SECRET` are stripped from the child environment. See [Security](/docs/reference/security) for the full model.
</Callout>
...
<Callout type="info">
Multi-agent discussions live alongside the engine in `packages/server/src/discussion/` — a Facilitator (Moderator) engine with a `turn-router`, prompt templates, and a registry. See [Discussions](/docs/guides/discussions).
</Callout>
...
For the user-facing canvas cards behind these mechanics — Output, Email, Condition, Structure, and Save to DB — see the [Data Nodes guide](/docs/guides/data-nodes).
...
When the server runs as root with `bypassPermissions`, it auto-sets `IS_SANDBOX=1` so the Claude Code CLI accepts the root invocation. See [Configuration](/docs/reference/configuration) for the complete environment reference.
