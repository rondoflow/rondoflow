...
<Callout type="info">
The `server` image installs the Claude Code CLI globally (`npm install -g @anthropic-ai/claude-code`) and adds `git`, so Assistants (agents) can start the CLI and the [Git panel](/guides/git) works inside the container.
</Callout>
...
| `OPENAI_API_KEY` / `PERPLEXITY_API_KEY` | Keys for the OpenAI / Perplexity agent [providers](/reference/configuration) | **Not in `.env.example`** — set in **Settings → Credentials** (encrypted) |

<Callout type="info">
`OPENAI_API_KEY` and `PERPLEXITY_API_KEY` are intentionally absent from `.env.example`. RondoFlow runs Assistants on three providers (`claude-code` is the default, plus `openai` and `perplexity`); the non-Claude keys are meant to be stored as encrypted credentials in **Settings → Credentials**, not in plaintext env. See [Configuration](/reference/configuration) for the provider model.
</Callout>
...
Use the `:ro` suffix for directories an Assistant should read but never modify. Only change `EXTERNAL_FOLDERS_CONTAINER_ROOT` if you also change the mount target in compose. See [External Folders](/guides/external-folders) for how folders surface in the workspace.
...
<Callout type="info">
The `server` container's `DATABASE_URL` and `migrate`'s `DATABASE_URL` both point at `postgres:5432` (the in-network hostname), not `localhost`. You do not normally need to override them. See [Configuration](/reference/configuration) for the full variable reference.
</Callout>
...
Sign in with that account, then invite everyone else from the **Users** panel. See [Users & Roles](/guides/users) for the role model and invite flow.
...
<Callout type="warning">
`IS_SANDBOX=1` tells the CLI to skip its root safety check, so a `bypassPermissions` Assistant can run unattended commands as root inside the container. Combined with mounted external folders this is powerful — scope your bind mounts (prefer `:ro`), keep the app behind authentication, and lean on [Safety Rules](/guides/safety-rules) to gate risky commands. See [Security](/reference/security) for the layered controls.
</Callout>
...
<Cards>
  ...
  <Cards.Card title="External Folders" href="/guides/external-folders" />
  <Cards.Card title="Users & Roles" href="/guides/users" />
  <Cards.Card title="Data Nodes (Email, Save to DB…)" href="/guides/data-nodes" />
