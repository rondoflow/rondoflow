...
<Callout type="info">
The Anthropic/Claude credentials live in the single root `.env` (and ship in `.env.example`). The OpenAI and Perplexity keys are **runtime-only credentials managed in Settings → Credentials** (encrypted in the database) — they are *not* in `.env.example` and aren't part of the [Configuration](/docs/getting-started/configuration) variable list. A manually-added `OPENAI_API_KEY` / `PERPLEXITY_API_KEY` environment variable still works as a fallback, but the supported path is Settings. See [Settings](/docs/guides/settings) for editing credentials in the UI.
</Callout>
...
<Cards>
  <Cards.Card title="Claude Code" href="/docs/providers/claude-code" />
  <Cards.Card title="OpenAI" href="/docs/providers/openai" />
  <Cards.Card title="Perplexity" href="/docs/providers/perplexity" />
  <Cards.Card title="Sakana AI" href="/docs/providers/sakana-ai" />
</Cards>
