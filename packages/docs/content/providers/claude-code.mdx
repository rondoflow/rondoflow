...
<Callout type="info">
Looking for the API-backed providers instead? See [OpenAI](/providers/openai) and [Perplexity](/providers/perplexity). Those talk to an HTTP API rather than the CLI and have a different config shape.
</Callout>
...
<Callout type="info">
RondoFlow also raises the per-response output ceiling for the child by setting `CLAUDE_CODE_MAX_OUTPUT_TOKENS` (default `128000`) so long results aren't truncated. The CLI clamps this to each model's true maximum, so a high value is safe across tiers. Override it in the root `.env` or via a workspace variable resource — see [Configuration](/reference/configuration).
</Callout>

## Tools and permission modes

Claude Code Assistants get **full tool access**, scoped by the allowed-tools list you grant and the [Safety Rules](/guides/safety-rules) attached to the Assistant or set globally.
...
Safety Rules can tighten this: an Assistant-level rule overrides a global one, and the most restrictive mode wins. See [Safety Rules](/guides/safety-rules) for resolution details.

<Callout type="warning">
`full` maps the CLI to `bypassPermissions` (`--dangerously-skip-permissions`), which the CLI refuses when running as root unless the environment is marked sandboxed. Server deployments often run as root in a container, so RondoFlow sets `IS_SANDBOX=1` automatically for exactly that case (root + bypass). See [Agents produce empty output](/faq#agents-produce-empty-output) if a headless run produces nothing.
</Callout>
...
Claude Code Assistants support [Connections](/guides/connections) (internally _MCP servers_). RondoFlow merges the Assistant's Connection configs (plus any from enabled skills) into a single `--mcp-config` JSON payload passed to the CLI. The serialized config can embed credentials, so it is redacted in debug logs.
...
- **Settings → Credentials** (recommended) — paste the value under the **Claude** (`anthropic`) group. It is stored AES-encrypted in the database, applies on save, and **overrides the `.env` value at boot** (loaded into the process environment at startup). See [Settings](/guides/settings).
...
Server secrets are explicitly stripped from anything a caller passes in — `DATABASE_URL`, `BETTER_AUTH_SECRET`, and `RONDOFLOW_SECRET` are never forwarded to a child, even via a workspace variable resource. See [Security](/reference/security).
...
See [Agents produce empty output](/faq#agents-produce-empty-output) for the full triage checklist.
