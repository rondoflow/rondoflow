...
OpenAI Assistants run on the same execution wiring as every other Assistant, so they slot into [Workflows](/docs/guides/workflows), [Orchestration](/docs/guides/orchestration), and [Discussions](/docs/guides/discussions) without any special handling. They differ only in what runs behind the scenes: an HTTP API call rather than a started (*spawned*) local process.

<Callout type="info">
OpenAI is one of three providers. See the [AI Providers](/docs/providers) overview for how providers compare, and [Claude Code](/docs/providers/claude-code) / [Perplexity](/docs/providers/perplexity) for the others.
</Callout>
...
- **Settings → Credentials** (recommended) — paste your key under the **OpenAI** group. It applies immediately on save. See [Settings](/docs/guides/settings).
...
Turn on **Web search** to add OpenAI's `web_search` tool to the run. When the model searches, RondoFlow surfaces the activity in the run feed as tool-use / tool-result events, so you can see when it reaches out to the web. (These are mapped from OpenAI's `response.web_search_call.in_progress` / `…searching` events to a `tool_use` and from `…completed` to a `tool_result`; no search query payload is captured.) See [Monitoring](/docs/guides/monitoring).
...
Claude-CLI-only concerns are skipped for API providers: the allowed-tools list, [Connections (*MCP Servers*)](/docs/guides/connections), [external folders](/docs/guides/external-folders), and permission modes do not apply to an OpenAI Assistant. Its only tools are the `web_search` / deep-research capabilities driven by `providerConfig`.
...
<Cards>
  ...
  <Cards.Card title="Perplexity provider" href="/docs/providers/perplexity" />
  <Cards.Card title="Settings & Credentials" href="/docs/guides/settings" />
  <Cards.Card title="Working with Assistants" href="/docs/guides/assistants" />
</Cards>
