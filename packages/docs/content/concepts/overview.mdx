...
This page is the conceptual map. Each concept below is defined in a few sentences and links to its full guide. RondoFlow uses friendly names in the UI (with the internal/technical term in parentheses on first mention); the full naming map lives in [Terminology](/concepts/terminology).

<Callout type="info">
  Want to see these concepts in action first? Jump to [Your First Workflow](/getting-started/first-workflow), then come back here to deepen your mental model.
</Callout>
...
See: [Assistants](/guides/assistants)
...
See: [Safety Rules](/guides/safety-rules)
...
See: [Discussions](/guides/discussions)
...
See: [Data Nodes](/guides/data-nodes) and [Workflows](/guides/workflows)
...
See: [Data Nodes](/guides/data-nodes)
...
| --- | --- |
| **Loop** | Lets an Assistant repeat until completion criteria are met, up to a max-iterations cap. See [Loops](/guides/loops). |
| **Orchestration** | The Planner, Director, and Advisor analyze, steer, and review runs. See [Orchestration](/guides/orchestration). |
| **External Folder** | Binds a host directory into the server so Assistants can read/write real files. See [External Folders](/guides/external-folders). |
| **Provider** | The engine behind an Assistant — Claude Code, OpenAI, or Perplexity. See [Providers](/providers). |
| **Roles & access** | The shared workspace is multi-user with global roles (admin / editor / viewer) and invite-only sign-up. See [Users & Roles](/guides/users). |
...
<Cards>
  <Cards.Card title="Terminology" href="/concepts/terminology" />
  <Cards.Card title="Your First Workflow" href="/getting-started/first-workflow" />
  <Cards.Card title="Working with the Canvas" href="/guides/canvas" />
  <Cards.Card title="Building Assistants" href="/guides/assistants" />
  <Cards.Card title="Data Nodes" href="/guides/data-nodes" />
</Cards>

For the complete map of user-facing names to internal terms, see [Terminology](/concepts/terminology).
